...
def get_hf_generation_args(request, tokenizer):
    ...
    # 停止序列用于使模型在期望的位置停止，例如句子或列表的结尾。通常情况下，回车键可作为单行完成的停止序列。
    # 停止序列是一个可选设置，用于告诉 API 何时停止生成令牌。完成的内容不会包含停止序列，您最多可以传递四个停止序列。
    # 如果没有传递任何信息，则默认为令牌的结束<|endoftext|>，这个令牌代表文本中可能的停顿点。
