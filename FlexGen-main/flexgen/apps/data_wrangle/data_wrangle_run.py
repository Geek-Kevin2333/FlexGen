...
# todo what's the purpose of this func and how is the out_put_ids being calculated
def single_query_test(args, task_instruction, test_data, task, pd_data_files, test_file):
    # Initialize environment
    tokenizer = get_tokenizer(args.model)
    # todo check ExecutionEnv
    ...
    for trial_num in range(args.num_trials):
        # initialise the random number generator, seed means the same result every time we run this file
        np.random.seed(args.seed + trial_num)
        queries = []
        # todo check promptUtils.
        ...
        preds = []
        idx = 0
        # 根据参数，产生prompt，再拼接上上text，然后进行tokenizer,生成了input_ids_tmp
