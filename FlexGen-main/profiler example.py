...
class GpipeAsync:
    ...
    def __init__(self, args, vocab_size, num_classes, device, use_dp=False, rank=None):
        ...
        # In parallel computing, rank refers to the unique identifier number of a process or worker in a group of interconnected processes called a "communicator".
        ...
        # comm is short for "communication" and refers to the communication library of parallel computing.
        #
        # In the GpipeAsync class, comm is a communication object that is used for parallelization across pipeline stages. Specifically, it is obtained using a function get_pipeline_parallel_comm().
        #
        # This object is used to send and receive data among the pipeline stages as well as to perform various communication and synchronization operations for the efficient implementation of the Gpipe parallelization technique.
        ...
        self.forward_comp_ready_events = [torch.cuda.Event(enable_timing=self.enable_tidy_profiling, blocking=False)
                                          for _ in range(self.micro_batch_num)]
        # For example, in the forward propagation method, self.forward_recv_ready_events[self.micro_batch_index].record() records an event that marks the point in time at which the forward receive phase is complete, allowing other pipeline stages to begin processing.
        ...
        # If the current pipeline stage is the first stage (i.e., self.pp_rank==0), self.input_micro_batches is set to None, since the input to the first stage is a single batch of size args.batch_size.
        if self.pp_rank == 0:
            self.input_micro_batches = None
        # otherwise, self.input_micro_batches is initialized with self.micro_batch_num tensors using the torch.zeros() function.
        # Each tensor has shape (self.micro_batch_size, self.seq_length, self.embedding_dim) corresponding to a micro-batch of self.micro_batch_size samples,
        # where each sample has self.seq_length time steps and an embedding dimension of `self.embedding_dim`.
        else:
        ...
        # Dynamic Precision (DP) is a technique used to improve the training of deep learning models
        # by reducing numerical errors that arise during the computation of gradients.
        # The basic idea behind DP is to use higher precision for aggregating gradients than for computing them.
        #
        # In DP, the model parameters are updated using accumulated gradients, and during the accumulation process,
        # lower precision can lead to numerically unstable gradients. Therefore, DP uses a higher precision (e.g., float32 or float64) for accumulation,
        # while using a lower precision (e.g., float16) for gradient computation.
        #
        # In other words, DP adds an additional precision stage to a model to compute aggregated gradients.
        # So instead of computing gradients in low-precision (such as float16), and then averaging them out and updating the parameters with the averaged gradients
        # (as done in the conventional gradient descent) in DP, for each mini-batch, the gradients are computed in low-precision, then accumulated in high-precision,
        # and finally, averaged out and updated the parameters with the accumulated gradients.
        #
        # This technique has become popular in deep learning training because it helps to improve the accuracy of the model
        # and stabilize the optimization process, especially when training models with large batch sizes or many layers.
        #
        # In the GpipeAsync class, the use_dp parameter is a flag to use dynamic precision, and if it is True,
        # an instance of a deep learning library designed for parallelism is created using a function get_dp_module().
        if use_dp:
    ...
    def forward_stage(self, input_data=None, target_data=None):
        ...
        if self.pp_rank == 0:
            ...
            # If the current stage is the first stage (self.pp_rank == 0), the input data input_data is split into self.micro_batch_num micro-batches using the torch.chunk() function,
            # and each micro-batch is assigned to the corresponding element of the self.input_micro_batches list.
            self.input_micro_batches = torch.chunk(input_data, self.micro_batch_num, dim=0)
        elif self.pp_rank == self.pipeline_group_size - 1:
            # If the current stage is the last stage (self.pp_rank == self.pipeline_group_size - 1), and the pipeline task is "Seq2SeqClassification",
            # the target_data is also split into self.micro_batch_num micro-batches using the torch.chunk() function.
        ...
        for i in range(self.micro_batch_num):
            # Inside the for loop, the actual computation takes place. There are three cases evaluated in the if-elif-else block,
            # depending on whether the current stage is the first, last, or intermediate stage of the pipeline.
    ...
    def profiling_forward_stage(self):
        """
        In the GpipeAsync class, profiling_forward_stage is a method for logging the execution times of the forward propagation subprocesses with respect to the start time of the computation.
        This method is called only if the flag self.enable_tidy_profiling is set to True in the GpipeAsync constructor. Tidy profiling allows the logging of subprocess execution times without any runtime overhead, unlike traditional profiling methods that might have a large overhead.
        Inside the profiling_forward_stage method, torch.cuda.synchronize() is called to synchronize with the CUDA stream in order to ensure that all events have completed. Then a for loop is executed over self.micro_batch_num.
        For each micro-batch, the elapsed time between the start event (forward_recv_start_events,forward_comp_start_events, and forward_send_start_events) and the ready event (forward_recv_ready_events,forward_comp_ready_events, and forward_send_end_events) is computed in milliseconds. Then the results of the elapsed time are appended to self.profiling_log, where micro-batch number and subprocess name are included as arguments. The if statements used in the for loop check the pipeline stage, in order to avoid profiling subprocesses that do not apply to a particular stage.
        The cname attribute in the log dictionary specifies the color to be used to display the event in visualization tools like Google's Chrome Trace Viewer.
        In summary, profiling_forward_stage logs the execution time of each subprocess in the forward propagation pipeline. This information can be used to profile the pipeline performance and identify potential bottlenecks in subprocesses.
        Tidy profiling is a profiling method that works by recording a timeline of subprocess events by inserting events that have no runtime overhead. This method allows for accurate profiling without intrusive runtime overhead while keeping the code tidy.
        The log generated by profiling_forward_stage can be visualized in tools like Chrome Trace Viewer to provide a graphical representation of the subprocess execution times.
        In conclusion, profiling_forward_stage is a useful method for Gpipe pipeline performance profiling that works without any additional runtime overhead. It can be used to identify potential bottlenecks in Gpipe pipelines by logging the elapsed time between subprocess start and completion events.
        """
        torch.cuda.synchronize()
        for i in range(self.micro_batch_num):
            if self.pp_rank != 0:
                # elapsed_time is a method of CUDA event objects that calculates the time elapsed between the start and end events recorded by the event objects.
                # In the GpipeAsync class, elapsed_time is used to calculate the time taken for subprocess execution between the start and end events.
                # For example, self.forward_comp_start_events[i].elapsed_time(self.forward_comp_ready_events[i]) calculates the time elapsed between self.forward_comp_start_events[i] and self.forward_comp_ready_events[i], which represents the time taken to execute the forward computation for the ith micro-batch.
                # The elapsed_time method returns the elapsed time in CUDA device time units, which are equivalent to a fraction of a second. To get an elapsed time in human-readable units (e.g., milliseconds), this value is multiplied by the constant 1e+3.
                # For example, elapsed_time(self.forward_comp_ready_events[i]) * 1e+3 would provide the elapsed time in milliseconds.
                # Overall, elapsed_time is a useful method for calculating time intervals between CUDA events and is used in the GpipeAsync class to profile subprocess execution times by computing the time elapsed between the start and end events of each subprocess.
