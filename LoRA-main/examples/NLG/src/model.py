...
# The Conv1D class is a PyTorch implementation of a one-dimensional convolutional layer that is commonly used in neural networks for natural language processing.
# This layer applies a linear transformation and element-wise activation function to the input tensor along a single dimension.
...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False):
        """
        @param nx: given input size
        @param n_ctx: the context length
        @param config: hyperparameters for this module
        The attention mechanism is implemented with n_head attention heads on the projected inputs, each of size n_state // n_head.
        @param scale:
        """
        ...
        # MergedLinear class is a wrapper for a PyTorch linear module that applies LoRA to the weights of the linear module.
    ...
    # todo 实际上也只是把产生key query value三个向量的过程进行了lora操作
...
class GPT2Model(nn.Module):
    def __init__(self, config):
        ...
        # The class first initializes the embeddings for the input tokens and the positional encodings using the nn.Embedding class.
        self.wte = nn.Embedding(config.vocab_size, config.n_embd)
        self.wpe = nn.Embedding(config.n_positions, config.n_embd)
        # It then creates a stack of Block objects (implemented as a nn.ModuleList), each of which contains a self-attention mechanism, a feed-forward neural network, and a residual connection.
    ...
    def forward(
        ...
        # In the forward pass of the model, the input_ids, position_ids, and token_type_ids are passed to the model as arguments.
        # The model then maps the input_ids to their corresponding embeddings using the self.wte embedding layer and applies the positional encodings and the token type embeddings.
        # The resulting embeddings are then passed through the stack of Blocks, and the output of the last block is fed through a LayerNorm before being returned.
        if past is None:
            past_length = 0
        ...
        # input_ids,token_type_ids,attention_mask这三个输入参数，这三个参数需要从输入文本中得到并正确设置，模型才能正确工作。
        # token_type_ids 也可以称为 segments_tensor 用来识别句子的界限。如果是单句子分类任务，默认为全0.如果是文本蕴含关系等句对任务，
        # 那么第一个句子对应的位置为全0，句子分隔符处置为0.第二个句子对应位置为全1
...
class GPT2LMModel(nn.Module):
    def __init__(self, config):
        ...
        # todo the difference between GPT2Model and GPT2LMHead
        # embedding weights 和 decoder weights开始是和GPT2Model一样的
...
# todo what is the meaning of past and len_past
    def forward(
        self, 
        input_ids, 
        lm_labels=None, 
        ...
        # todo what is presents. 经过原来的模型之后，还需要经过lm_head的计算
