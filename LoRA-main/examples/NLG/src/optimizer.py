...
class AdamW(Optimizer):
    ...
    def step(self, closure=None):
        ...
        if closure is not None:
            loss = closure()
        # The step function iterates over the parameter groups, which are defined by the user when creating the optimizer.
        # For each parameter in each group, the gradient is computed using the backward function of PyTorch's autograd framework,
        # and stored in the grad attribute of the parameter. If the gradient is None, it is skipped.
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if len(state) == 0:
                    ...
                    state["exp_avg_sq"] = torch.zeros_like(p.data)
                # For each parameter, the optimizer maintains two exponential moving averages of gradients: exp_avg and exp_avg_sq.
                # These averages are initialized to zero using state dictionary. The optimizer uses beta1 and beta2 hyperparameters to control the decay rate of the moving averages, and a small epsilon value to avoid division by zero.
                #
                # The optimizer updates the moving averages and computes the adaptive learning rate using the current step number, the beta parameters, and the epsilon value.
                # It then updates the parameters using the learning rate and the moving averages.
                #
                # Finally, the optimizer applies weight decay to the parameters using a separate hyperparameter.
                # It subtracts the product of the learning rate and the weight decay from each parameter value.

                # Maintaining exponential moving averages of gradients helps to smooth out the noise and variance in the estimated gradients and provides a better estimate of the true gradient direction. This helps to avoid getting stuck in local minima or saddle points and improves the chances of finding the global minimum of the loss function.
                # The beta1 and beta2 hyperparameters control the decay rate of the moving averages: beta1 controls the decay rate of the first moment (the mean of the gradients) and beta2 controls the decay rate of the second moment (the uncentered variance of the gradients). Choosing appropriate values for beta1 and beta2 influences the speed of convergence and the stability of the optimization.
                #
                # The small epsilon value is added to the denominator when computing the adaptive learning rate to avoid division by zero. This is important because the denominator can be very small when the moving average of squared gradients is close to zero, which can cause the learning rate to become very large and lead to unstable updates.
                # Using adaptive learning rates based on the moving averages of gradients helps to dynamically adjust the learning rate for each parameter depending on the smoothness and magnitude of the gradients. This helps to achieve faster convergence and avoid oscillations or overshooting of the optimal value.
                #
                # Overall, these steps of maintaining moving averages of gradients and computing adaptive learning rates make the Adam optimizer more robust and effective for a wider range of optimization problems.
